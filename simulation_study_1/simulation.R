...
for(N in unique(simulations$N)) {
  dir <- glue::glue("/gpfs/scratch/susmah01/PenalizedCausalInference/covariance_cache/{N}")
  if(!dir.exists(dir)) {
    dir.create(dir)
  }
}
 
filename <- glue::glue("/gpfs/scratch/susmah01/PenalizedCausalInference/covariance_cache/{index}.rds")
y <- "Y"
